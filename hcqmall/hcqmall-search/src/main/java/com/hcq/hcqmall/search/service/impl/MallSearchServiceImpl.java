...
import com.hcq.hcqmall.search.feign.ProductFeignService;
import com.hcq.hcqmall.search.service.MallSearchService;
import com.hcq.hcqmall.search.vo.AttrResponseVo;
import com.hcq.hcqmall.search.vo.BrandVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //2、执行检索请求
            SearchResponse response = client.search(searchRequest, hcqmallElasticSearchConfig.COMMON_OPTIONS);
            //3、分析响应数据封装成我们需要的格式
...
//    private SearchResult searchByHu(SearchParam param){
//        SearchResult result=null;
//        SearchRequest searchRequest=buildSearchRequestByHu(param);
//        try {
//            SearchResponse response = client.search(searchRequest, hcqmallElasticSearchConfig.COMMON_OPTIONS);
//            //分析响应数据
//            result = buildSearchResultByHu(response);
//        } catch (IOException e) {
...
//    }
//
//    private SearchResult buildSearchResultByHu(SearchResponse response) {
//        SearchResult result = new SearchResult();
//        SearchHits hits = response.getHits();
...
//    }
//
//    private SearchRequest buildSearchRequestByHu(SearchParam param) {
//        SearchSourceBuilder sourceBuilder=new SearchSourceBuilder();//构建dsl语句
//        //模糊匹配，过滤
//        //1.构建布尔builder
//        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
...
//            BoolQueryBuilder nestedBoolQuery = QueryBuilders.boolQuery();
//
//            //            attrs=1_5寸：8寸&attrs=2_16g:8
//            for (String attrStr : param.getAttrs()) {
//                String[] s = attrStr.split("_");
//                String attrId=s[0];
//                String[] attrValue = s[1].split(":");
//                nestedBoolQuery.must(QueryBuilders.termQuery("attrs.attrId",attrId));
//                nestedBoolQuery.must(QueryBuilders.termsQuery("attr.attrValue",attrValue));
//                NestedQueryBuilder nestedQuery = QueryBuilders.nestedQuery("attrs", nestedBoolQuery, ScoreMode.None);
//                boolQuery.filter(nestedQuery);
//            }
//        }
